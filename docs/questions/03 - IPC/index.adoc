...
image::images/Publish-subscribe simple example.png[]
...
image::images/Closed circuit breaker.png[]
...
image::images/Open circuit breaker.png[]
...
.Bulkhead pattern
image::images/Bulkhead pattern.png[]
...
The sidecar pattern is a mechanism that allows a service to add additional functionality to a microservice, such as logging, monitoring, or authentication. It is typically implemented as a separate process that runs alongside the microservice. Basically, the sidecar pattern is composed of two parts: the microservice and the sidecar.

The following diagram shows a simple example of the sidecar pattern:

.Sidecar pattern
image::images/Sidecar pattern.png[]

In this example, each microservice is composed of two parts: the microservice and the sidecar. When a request is made from or to the microservice, the sidecar intercepts the request and adds additional functionality, then forwards the request.
...
.Istio service mesh
image::images/Istio service mesh.png[]
...
.Self-registration
image::images/Self-registration.png[]
...
.Third-party registration
image::images/Third-party registration.png[]
...
.Server-side discovery
image::images/Server-side discovery.png[]
...
For our application Polycode, we have multiples types of microservices.

Pattern ?
Mailer différent avec une event queue
Rest partout
